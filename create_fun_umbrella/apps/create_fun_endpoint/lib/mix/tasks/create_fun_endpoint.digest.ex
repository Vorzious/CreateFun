defmodule Mix.Tasks.CreateFunEndpoint.Digest do
  use Mix.Task
  @default_input_path ["priv/static"]
  @default_output_path "priv/static"
  @app_whitelist [:create_fun_endpoint]
  ...
  def run(["default"]) do
    run([
        "../create_fun_cms/priv/static:cms", #YipYip-Scaffold-Remove-CMS
         "../create_fun_web/priv/static", #YipYip-Scaffold-Remove-WEB
         "-o", "../create_fun_endpoint/priv/static"
         ])
  end
  def run(all_args) do
    if Enum.member?(@app_whitelist, Mix.Project.config()[:app]) do
      ...
      case CreateFunEndpoint.Digester.compile(input_paths, output_path) do
        :ok ->
