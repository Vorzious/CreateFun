...
<!-- These settings can be found in `create_fun_umbrella/apps/create_fun_cms/assets/vendor/js/plugins/dataTables/yy_datatables.js` -->
```
...
def render("for_datatables.json", %{data: data}) do
    %{
        ...
        data: render_many(data.data, CreateFunCms.FooView, "foo.json")
    }
end
  ...
  def render("foo.json", %{foo: foo}) do
    %{
      foo_name: foo.name,
      foo_age: foo.age,
      show_url: link(Gettext.dgettext(CreateFunCms.Gettext, "default", "Show"),
        [to: foo_path(CreateFunEndpoint.Endpoint, :show, foo), class: "btn btn-default btn-xs"])
        |> safe_to_string() ,
      edit_url: link(Gettext.dgettext(CreateFunCms.Gettext, "default", "Edit"),
        [to: foo_path(CreateFunEndpoint.Endpoint, :edit, foo), class: "btn btn-default btn-xs"])
        |> safe_to_string() ,
      delete_url: link(Gettext.dgettext(CreateFunCms.Gettext, "default", "Delete"),
        [to: foo_path(CreateFunEndpoint.Endpoint, :delete, foo), class: "btn btn-danger btn-xs",
